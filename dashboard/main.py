...
async def download_icloud_package(request: Request):
    ...
    try:
        ...
        adapter_config = {
            ...
            "local_tm_path": config.get('local_path', CASE_DIRECTORY),
            "sync_interval": config.get('sync_interval', 30),
        ...
        with tarfile.open(mode='w:gz', fileobj=buffer) as tar:
            ...
            readme_content = f"""# TM iCloud Sync Adapter v1.0.0
...
- **Local TM Path**: {adapter_config['local_tm_path']}
- **Sync Interval**: {adapter_config['sync_interval']} seconds
- **Log Level**: {adapter_config['log_level']}
- **Backup Enabled**: {adapter_config['backup_enabled']}
...
---
Generated by TM Dashboard v{APP_VERSION} on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
""".encode('utf-8')
            ...
            install_info = tarfile.TarInfo(name='INSTALL.md')
            install_content = f"""# TM iSync Adapter Installation Instructions
...
```json
{{
    "icloud_parent_folder": "{adapter_config['icloud_parent_folder']}",
    "local_tm_path": "{adapter_config['local_tm_path']}",
    "sync_interval": {adapter_config['sync_interval']},
    "log_level": "{adapter_config['log_level']}",
    "backup_enabled": {str(adapter_config['backup_enabled']).lower()}
}}
```
...
""".encode('utf-8')
            install_info.size = len(install_content)
            tar.addfile(install_info, io.BytesIO(install_content))
            logger.info("Added installation instructions to package")
