...
# Clear main case output directory (outputs/tests/<case_name>)
if [[ -d "$CASE_OUTPUT_DIR" ]]; then
    ...
    echo "ℹ️  No case output directory found in outputs/tests for '$CASE_NAME'"
fi

# Clear dashboard output directory (dashboard/outputs/<case_name>)
DASHBOARD_OUTPUT_DIR="$TM_ROOT/dashboard/outputs/$CASE_NAME"
if [[ -d "$DASHBOARD_OUTPUT_DIR" ]]; then
    echo "Removing dashboard output files from: $DASHBOARD_OUTPUT_DIR"
    rm -rf "$DASHBOARD_OUTPUT_DIR"
    echo "✅ Dashboard output directory cleared"
else
    echo "ℹ️  No dashboard output directory found for '$CASE_NAME'"
fi
...
echo "   - All dashboard files removed from dashboard/outputs/$CASE_NAME"
...
echo "If dashboard doesn't update, click the 'Refresh' button or hard refresh browser."